...
class RFDBCTrustedZone:
	...
	def remove_rows(self, df: DataFrame) -> DataFrame:
		...
		if value_col in current_df.columns:
			# First, calculate and print the actual min and max before filtering
			print(f"\nAnalyzing column: '{value_col}'")
			stats = current_df.agg(
				F.min(value_col).alias("actual_min"),
				F.max(value_col).alias("actual_max")
			).first()

			if stats and stats["actual_min"] is not None:
				# Using .2f to format the numbers nicely
				print(f"  > Found Min Value: {stats['actual_min']:.2f}, Max Value: {stats['actual_max']:.2f}")
			else:
				print(f"  > Column '{value_col}' is empty or contains only NULL values.")
			
			null_or_nan_count = current_df.filter(F.col("value").isNull() | F.isnan(F.col("value"))).count()
			print(f"  > Found {null_or_nan_count} NULL or NaN values.")

			# Now, apply the constraint as before
			min_val = 0
			max_val = 999999999
			
			print(f"Applying constraint: '{value_col}' must be between {min_val} and {max_val}.")
			
			value_condition = F.col(value_col).between(min_val, max_val)
			
			df_filtered = current_df.filter(value_condition)
			count_after_filter = df_filtered.count()
			rows_removed = rows_before_constraints - count_after_filter
			
			print(f"- Removed {rows_removed} rows violating the value range constraint [{min_val}, {max_val}].")
			print(f"  Rows remaining: {count_after_filter}")
			
			current_df = df_filtered
			rows_before_constraints = count_after_filter
		else:
			print(f"Warning: Column '{value_col}' not found. Skipping value range constraint.")
